program test
   use gghh_config, only: ki, logfile, nlo_prefactors
   use gghh_kinematics, only: dotproduct, boost_to_cms
   use gghh_model, only: parse
   use gghh_matrix, only: samplitude, &
     & initgolem, exitgolem, ir_subtraction
   use gghh_color, only: numcs, CA
   use gghh_rambo, only: ramb
...
subroutine  print_parameters(scale2)
   use gghh_config, only: renormalisation, &
