module     gghh_model
   ! Model parameters for the model: FeynRules,models/EWChL_UFO
   ...
   real(ki) :: mdlaS =        0.118400000000000_ki
   ...
   real(ki) :: mdlMB =        4.700000000000000_ki
   real(ki) :: mdlMC =        1.270000000000000_ki
   real(ki) :: mdlMD =        0.005040000000000_ki
   real(ki) :: mdlMe =        0.000511000000000_ki
   real(ki) :: mdlMMU =        0.105660000000000_ki
   real(ki) :: mdlMS =        0.101000000000000_ki
   real(ki) :: mdlMT =      172.000000000000000_ki
   real(ki) :: mdlMTA =        1.777000000000000_ki
   real(ki) :: mdlMU =        0.002550000000000_ki
   real(ki) :: mdlMZ =       91.187600000000003_ki
   real(ki) :: mdlWh =        0.004070000000000_ki
   real(ki),parameter :: mdlWT =        0.000000000000000_ki
   real(ki) :: mdlWW =        2.085000000000000_ki
   real(ki) :: mdlWZ =        2.495200000000000_ki
   real(ki) :: mdlymb =        4.700000000000000_ki
   ...
   real(ki) :: mdlymt =      172.000000000000000_ki
   ...
   character(len=name_length), dimension(42) :: names = (/&
      & "gauge1z  ", &
      & "gauge2z  ", &
      & "mdlaEWM1 ", &
      & "mdlaS    ", &
...
!---#[ print_parameter:
   ...
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlaS= ", mdlaS
   ...
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlMB= ", mdlMB
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlMC= ", mdlMC
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlMD= ", mdlMD
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlMe= ", mdlMe
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlMMU= ", mdlMMU
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlMS= ", mdlMS
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlMT= ", mdlMT
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlMTA= ", mdlMTA
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlMU= ", mdlMU
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlMZ= ", mdlMZ
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlWh= ", mdlWh
   write(unit,'(A1,1x,A11,G23.16,"const.")') "#", "mdlWT= ", mdlWT
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlWW= ", mdlWW
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlWZ= ", mdlWZ
   write(unit,'(A1,1x,A11,G23.16)') "#", "mdlymb= ", mdlymb
   ...
   subroutine     parseline(line,stat,line_number)
      ...
      elseif (name(1:7).eq."yukawa(") then
         ...
         select case(pdg)
            ...
            case(5)
               mdlymb = parsereal(value, ierr, lnr)
            case(6)
      ...
      elseif (name(1:9).eq."sminputs(") then
         ...
         select case(pdg)
            ...
            case(3)
               mdlaS = parsereal(value, ierr, lnr)
            case default
      ...
      elseif (name(1:5).eq."mass(") then
         ...
         select case(pdg)
            case(1)
               mdlMD = parsereal(value, ierr, lnr)
            case(2)
               mdlMU = parsereal(value, ierr, lnr)
            case(3)
               mdlMS = parsereal(value, ierr, lnr)
            case(4)
               mdlMC = parsereal(value, ierr, lnr)
            case(5)
               mdlMB = parsereal(value, ierr, lnr)
            case(6)
               mdlMT = parsereal(value, ierr, lnr)
            case(11)
               mdlMe = parsereal(value, ierr, lnr)
            case(13)
      ...
      elseif (name(1:6).eq."decay(") then
         ...
         select case(pdg)
            case(24)
               mdlWW = parsereal(value, ierr, lnr)
            case(5000000)
               mdlWh = parsereal(value, ierr, lnr)
            case(23)
               mdlWZ = parsereal(value, ierr, lnr)
            case default
      ...
      loop1: do
         ...
         if (line(1:1) .eq. 'B') then
            if (line(1:5) .eq. 'BLOCK') then
               ...
               l = i
           if ("YUKAWA" .eq. line(1:l)) then
                  block = 0
           else           if ("SMINPUTS" .eq. line(1:l)) then
                  block = 1
           else           if ("FRBLOCK" .eq. line(1:l)) then
                  block = 2
...
if (name(1:5).eq."mass(" .and. len_trim(name)>=7) then
     ...
     elseif (name .eq. "SMINPUTS&&2") then
               must_be_real = .true.
               mdlGf = re
     elseif (name .eq. "SMINPUTS&&3") then
               must_be_real = .true.
               mdlaS = re
     ...
     elseif (name .eq. "DECAY&&24") then
               must_be_real = .true.
               mdlWW = re
     elseif (name .eq. "DECAY&&5000000") then
               must_be_real = .true.
               mdlWh = re
