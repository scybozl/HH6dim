module     gghh_smehc_dipoles
   use gghh_smehc_config, only: ki
   use gghh_smehc_color, only: gammaF, gammaA, &
       & CF, CA, numcs, KF, KA, &
       & T1T2
   use gghh_smehc_kinematics, only: num_legs, dotproduct, &
       lambda
   implicit none
...
contains
   ...
   function I_ff_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(mH, ki), real(mH, ki))
   ...
   function I_if_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ncs=1.0_ki
              term = V_SING_QED(s_ja, mH, 0.0_ki)
