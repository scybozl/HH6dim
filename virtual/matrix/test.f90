program test
   use gghh_smehc_config, only: ki, logfile, nlo_prefactors
   use gghh_smehc_kinematics, only: dotproduct, boost_to_cms
   use gghh_smehc_model, only: parse
   use gghh_smehc_matrix, only: samplitude, &
     & initgolem, exitgolem, ir_subtraction
   use gghh_smehc_color, only: numcs, CA
   use gghh_smehc_rambo, only: ramb
...
subroutine  print_parameters(scale2)
   use gghh_smehc_config, only: renormalisation, &
