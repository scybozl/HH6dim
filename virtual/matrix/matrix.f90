module     gghh_smehc_matrix
   use gghh_smehc_util, only: square
   ...
   use gghh_smehc_kinematics, only: &
       in_helicities, symmetry_factor, num_legs, &
       lo_qcd_couplings, corrections_are_qcd, num_light_quarks, num_gluons
   use gghh_smehc_model, only: Nf, NC, sqrt2, init_functions
   use gghh_smehc_color, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use gghh_smehc_diagramsh0l0, only: amplitude0l0 => amplitude
   use gghh_smehc_amplitudeh0, only: samplitudeh0l1 => samplitude, &
        &   finite_renormalisation0 => finite_renormalisation
   use gghh_smehc_dipoles, only: insertion_operator, insertion_operator_qed
...
contains
   ...
   subroutine     banner()
      ...
      write(banner_ch,'(A74)') "|  \__/ \__/ (___/(_)(_)(_/\/\_)        Version 2.0.4 Rev: 12f4de9       |"
      write(banner_ch,'(A74)') "|                                                                        |"
   ...
   subroutine     samplitude(vecs, scale2, amp, prec, ok, h)
      ...
      if(PSP_check) then
         ...
         if(icheck.eq.3.and.PSP_verbosity) then
            ...
            write(42,'(4x,A15,A10,A3)') &
                 &  "<process name='","gghh_smehc","'/>"
   ...
   subroutine     samplitudel01(vecs, scale2, amp, rat2, ok, h)
      ...
      use gghh_smehc_dipoles, only: pi
      implicit none
      real(ki), dimension(4, 4), intent(in) :: vecs
      real(ki), intent(in) :: scale2
   ...
   function     samplitudel0(vecs, h) result(amp)
      use gghh_smehc_config, only: logfile
      use gghh_smehc_kinematics, only: init_event
      implicit none
      real(ki), dimension(4, 4), intent(in) :: vecs
   ...
   pure subroutine color_correlated_lo(color_vector,res)
      use gghh_smehc_color, only: T1T1, &
      & T1T2, &
      & T2T2
      implicit none
   ...
   pure function square_0l_0l_sc(color_vector1, color_vector2) result(amp)
      use gghh_smehc_color, only: cmat => CC
      implicit none
   ...
   pure function square_1_2_sc(color_vector1, color_vector2) result(amp)
      use gghh_smehc_color, only: cmat => T1T2
