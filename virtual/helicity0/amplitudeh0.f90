module    gghh_smehc_amplitudeh0
   use gghh_smehc_config, only: ki, &
       & reduction_interoperation
   use gghh_smehc_color, only: numcs
   use gghh_smehc_groups
   use ninjago_module, only: ki_nin
   use gghh_smehc_ninjah0
...
!---#[ function finite_renormalisation:
   function     finite_renormalisation(scale2) result(amp)
      ...
      use gghh_smehc_color, only: CF, CA
   ...
   function     samplitude(scale2,ok,rational2,opt_amp0,opt_perm)
      use gghh_smehc_config, only: include_eps_terms, include_eps2_terms, &
...
subroutine     evaluate_group0(scale2,samplitude,ok)
   ...
   use gghh_smehc_ninjah0, only: ninja_reduce => ninja_reduce_group0
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
