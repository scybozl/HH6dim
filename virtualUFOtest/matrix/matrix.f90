module     gghh_matrix
   ...
   use gghh_model, only: Nf, NC, sqrt2, init_functions
   use gghh_color, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use gghh_diagramsh0l0, only: amplitude0l0 => amplitude
   use gghh_amplitudeh0, only: samplitudeh0l1 => samplitude, &
        &   finite_renormalisation0 => finite_renormalisation
   use gghh_dipoles, only: insertion_operator, insertion_operator_qed
...
contains
   ...
   function     samplitudel0(vecs, h) result(amp)
      use gghh_config, only: logfile
      use gghh_kinematics, only: init_event
      implicit none
      real(ki), dimension(4, 4), intent(in) :: vecs
